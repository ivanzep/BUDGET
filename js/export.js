...
export function exportCSV(project) {
  const rows = [
    ['Section', 'Version', 'Dev Cost Code', 'Category', 'Subcategory', 'Item ID', 'Description', 'Cost Type', 'Area', 'Unit', 'Unit Cost Material', 'Unit Cost Labor', 'Markup %', 'Qty', 'Notes', 'Line Total'],
  ];
  project.info.versions.forEach((v) => {
    linesForVersion(project.lines, v.id).forEach((l) => {
      rows.push([
        'Budget', v.name, l.devCostCode || '', l.category, l.subcategory, l.itemId, l.description, l.costType || '', areaName(project, l.areaId), l.unit,
...
export function exportExcel(project) {
  ...
  versions.forEach((v) => {
    const header = ['Dev Cost Code', 'Category', 'Subcategory', 'Item ID', 'Description', 'Cost Type', 'Area', 'Unit', 'Unit Cost Material', 'Unit Cost Labor', 'Markup %', 'Qty', 'Notes', 'Line Total'];
    const rows = linesForVersion(project.lines, v.id).map((l) => [
      l.devCostCode || '', l.category, l.subcategory, l.itemId, l.description, l.costType || '', areaName(project, l.areaId), l.unit, Number(l.unitCostMaterial) || 0,
